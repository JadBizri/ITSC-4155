...
export default function Create() {
	...
	const item = api.item.getItemSlug.useQuery(slug);
	...
	const mutation = api.item.editItem.useMutation();

	async function onSubmit(data: z.infer<typeof formSchema>) {
		const res = await mutation.mutateAsync({ ...data, slug });
		form.reset();
		await router.push(`/product/${res.slug}`);
	}
	if (item.isLoading) return <div>Loading...</div>;

	const authorized = sessionData && item.data?.createdBy.id === sessionData.user.id;
	...
	return (
		<>
			{authorized ? (
				...
				<div>Log in to edit the listing</div>
			)}
		</>
	);
}
